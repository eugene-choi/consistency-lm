...
user_folders = {
    'output-dirname': {
        'ec2684': '/scratch/ec2684/research/consistency-lm/runs',
    },
    'base-dir': {
        'ec2684': '/scratch/ec2684/research/consistency-lm'
    }
}
...
for idx, job in enumerate(jobs):
    ...
    singularity = f'''singularity exec --nv --overlay $SCRATCH/singularity/overlay-25GB-500K-0{idx}.ext3:ro /scratch/work/public/singularity/cuda11.3.0-cudnn8-devel-ubuntu20.04.sif  /bin/bash -c "\nsource /ext3/env.sh\nconda activate\n'''
    cmd = [singularity,'python3', '-u', script]
    ...
    with open(slurm_script_path, 'w') as slurmfile:
        ...
        if args.partition == 'partition_name':
            slurmfile.write("#SBATCH --mem=64G\n")
            slurmfile.write("#SBATCH --gres=gpu:rtx8000:%d\n" % args.ngpus)
        else:
            raise NotImplementedError(args.partition)
        slurmfile.write("#SBATCH -c %d\n" % args.ncpus)
        
        # email
        slurmfile.write("#SBATCH --mail-user=<ec2684@nyu.edu>\n")
