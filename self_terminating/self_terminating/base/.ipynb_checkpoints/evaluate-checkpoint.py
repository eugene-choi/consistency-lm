    ...
    for d in dirs:
        ...
        if not args.use_last:
            ...
            if torch.cuda.is_available():
                ...
                ckpt = torch.load(os.path.join(d, "model_best.pt"), map_location=torch.device('cpu'))
        else:
        ...
        if nested_args:
            ...
            print('NOT nested_args')
            options = model_args.__dict__
            nested_args = model_args
        print(args.one_eos)
        # print(options['one_eos'])
        # model, criterion, _ = utils.setup_rnn(vocab, args.one_eos, **options)
        ...
        if args.dataset_version == "wikitext_sentencized":
            raw_datasets, datasets, _, stats = data.wikitext_sentencized(
                args.dataset_path, model_args.mask_context_k,args.num_eos
            )
...
def evaluate(
    ...
    print(f'''isinstance(model, utils.STRLMPretrainFinetune) = {isinstance(model, utils.STRLMPretrainFinetune)}''')
    if isinstance(model, utils.STRLMPretrainFinetune):
    ...
    print(f'''isinstance(model, utils.RNNLanguageModelST) = {isinstance(model, utils.RNNLanguageModelST)}''')
...
if __name__ == "__main__":
    ...
    parser.add_argument("--one-eos", action='store_true')
